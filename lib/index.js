...
var MailchimpExport = function () {
  ...
  _createClass(MailchimpExport, [{
    ...
    key: 'campaignSubscriberActivityBulk',
    value: function campaignSubscriberActivityBulk(params) {
      var _this2 = this;

      return new Promise(function (resolve, reject) {
        var subscriberList = [];
        _this2.getFromExportApi('campaignSubscriberActivity', params).on('data', function (chunk) {
          return subscriberList.push(JSON.parse(chunk.toString('utf8')));
        }).on('complete', function () {
          return resolve(subscriberList);
        }).on('error', reject);
      });
    }
  }, {
